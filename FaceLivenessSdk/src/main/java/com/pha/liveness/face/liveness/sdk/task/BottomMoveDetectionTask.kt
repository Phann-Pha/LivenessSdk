...
class BottomMoveDetectionTask : LivenessDetectionTask
{
    private var hasMoveToBottom = false

    override var isTaskCompleted = false

    override fun taskType(): String = DetectionConstance.MOVEMENT_BOTTOM_DETECTION

    override fun start()
    {
        hasMoveToBottom = false
    }
    ...
    override fun process(imageQuality: ImageQualityAnalyzer, occlusionDetector: FaceOcclusionDetector, livenessDetector: LivenessDetector, bitmap: Bitmap?, face: Face, timestamp: Long): Boolean
    {
        ...
        {
            if (angle.down() || angle.superDown())
            {
                hasMoveToBottom = true
            }
        }
        return hasMoveToBottom
    }
}